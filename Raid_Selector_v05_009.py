...
def raidReport(cnt_tank, cnt_healer, cnt_melee, cnt_ranged):
    f = open("hello.txt", "a")
    report = (("Tank count: {}, Healer count: {}, Melee count: {}, Ranged count: {}").format(cnt_tank, cnt_healer, cnt_melee, cnt_ranged))
    f.write("".join(report))
    f.close()
            ...
            try:
                ...
                if write_mode == 0:
                    with open("hello.txt", "a") as f:
                        ...
                        print(terminus)
                        print("{} line(s) output to file.".format(cycle_count))
                        #f.write("{} line(s) output to file.".format(cycle_count))
    ...
    while True:
        try:
            ...
            if cycle_count < 0:
                ...
                for cyc in range(cycle_count):
                    role_sample = (random.choices(role_list, k=1))
                    class_sample = (random.choices(class_list, k=1))

                    out_class = ("".join(class_sample))
                    out_role = ("".join(role_sample))
                    # out_spec = ("".join(spec_sample))

                    file_out = ("Member {:03}: {} {:<10} {:<10}\n".format((cyc + 1), " >> ", out_role, out_class))
...
#TODO 20220806 - cnt_* variables are logged correctly at write stage,  not joined into terminus correctly
#TODO 20220906 - cnt variables from terminus are not able to be read from outside, can be parsed from inside write
#TODO f.write inside overwrite mode is overwriting first two lines of file, include terminus printing inside write file to avoid issue?
#TODO add in 'output line(s) to file' statment before terminus
#TODO add in raidReport function (below terminus)
#TODO 20220614 create method getTime to insert into classpicker sequence, f.write from outside write_file is overwriting existing line_output
#TODO 20220615 raidReport, terminus, timestamp have overwriting problem
#TODO 20220616 progress on timestamp, overwriting problem with raidReport to fix